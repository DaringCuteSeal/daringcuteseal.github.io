...
![An image demonstrating that different left-bottom corner point location determines whether point A or B is better](../blog/image/equal-cornerness-example.png)
...
![An image showing a rectangle formed from two corner points that are part of the convex hull set of a bunch of points](../blog/image/rectangle-part-of-convex-hull.png)
...
0 & \text{if node has no ingoing edges and isn't starting point}\\
...
\sum \text{paths} = (\sum \text{svr} \rarr \text{dac})(\sum \text{dac} \rarr \text{fft}) (\sum \text{fft} \rarr \text{out}) +\\
(\sum \text{svr} \rarr \text{fft})(\sum \text{fft} \rarr \text{dac}) (\sum \text{dac} \rarr \text{out})
...
Anyway, this problem is ridiculous. This is NP-hard. If there exists a computationally cheap solution, the Tetris champions would absolutely go nuts over it.
